...
# hyperparameters and constants for features
...
# Main feature extraction function for a single well. Returns dict of features or None if failed.
 
def build_well(hw, tw, spatial_knn, dense_imputer, wid="unknown_well", is_train=False):
    ...
    d_ancc, d_std, d_dist = dense_imputer.impute(xy_ev, self_wid=swid)
    d_kn, d_std_kn, _ = dense_imputer.impute(xy_kn, self_wid=swid)
    b_vd = ktvt + z_kn - d_kn
